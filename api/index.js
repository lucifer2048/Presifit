...
app.post('/steps', async (req, res) => {
  const { userId, steps, distanceCovered, caloriesBurned } = req.body;
  ...
  try {
    ...
    res.status(200).json({ message: 'User steps saved successfully' });
  } catch (error) {
...
app.get('/steps', async (req, res) => {
  try {
    const userSteps = await Steps.findById({ userId: req.query.userId });

    res.status(200).json(userSteps);
  } catch (error) {
    res.status(500).json({ message: 'Server error', error });
  }
});
